...
`terraform-provider-infrawrench/` at the repo root manages **Infrawrench's own FinOps configuration** as Terraform resources: budgets, cost centres, allocation rules, tag policy, saved filters, cost reports and folders, cost alerts, scenario models, billing rules, cost exports (11 resources, 3 data sources). Its README carries the full design rationale; what follows is what binds changes made elsewhere in the repo.

Three unrelated features have "Terraform" in the name — **eject to Terraform** (`Plugin.terraformExport`, writes HCL describing _the user's cloud resources_), **org config as code** (`infrawrench config export/plan/apply`, whole-org JSON document), and **this provider** (per-object CRUD against the cost routes). The provider deliberately does not wrap config as code: that document addresses entities by a name-derived `key` with no server id, so import is impossible and a rename becomes destroy-and-recreate.

- **It is outside every JS toolchain, on purpose.** Not in `pnpm-workspace.yaml`, not in `turbo.json`, not in `cliff.toml`'s `include_paths` (a standalone Go module is outside the desktop changelog's dependency closure). Build and test it on its own: `cd terraform-provider-infrawrench && go build ./... && go test ./...`, Go 1.25+.
- **Every URL and JSON body lives in `internal/iw/`** — `wire.go` for the structs, `endpoints.go` for typed per-object methods. Nothing under `internal/provider/` assembles a path or a body. These are hand-written rather than generated from the spec because the SDK generator collapses `discriminator` unions to `any` and drops `readOnly`/`default`, which are exactly the signals that decide whether an attribute is `Computed`.
- **A change to a cost/FinOps route is a change to the provider, in the same commit** — the same mirroring rule that holds between web, desktop and mobile. `internal/iw/wire_spec_test.go` reads `app/packages/web/openapi.json` and fails when a schema property is neither decoded by a Go struct nor listed in that check's `ignored` list with a written reason. It runs one way only (extra Go fields are expected, since the checked-in spec lags the routes) and skips when the spec is absent, which keeps the module extractable to its own repository.
- **Regenerating `openapi.json` can fail the provider's tests, by design.** `schemasKnownAbsent` names seven schemas — scenario models, billing rules and their children — whose routes exist but which the spec has never carried. When they appear, `TestKnownAbsentSchemasAreStillAbsent` fails and tells whoever regenerated the spec to move them into `specChecks` so they start being covered.
- **Every resource must implement `ImportState` and `Configure`**; `provider_test.go` enforces both across the registry, because someone adopting the provider already has budgets and a resource without import would have to be deleted and recreated to come under management. Objects with no single-GET route (cost centres, allocation rules, report folders) are read by listing and filtering, and their `endpoints.go` wrapper synthesises the 404 itself so an out-of-band delete still surfaces as "needs recreating" rather than silent success.
- **Adding a resource or data source** touches, in one change: `wire.go` + `endpoints.go`, the `internal/provider/resource_*.go` (or `datasource_*.go`) file, the registry in `provider.go`, the expected counts in `schema_validation_test.go`, a `specChecks` entry when the spec describes the schema, the README's resource table, and `app/packages/website/src/content/docs/features/terraform-provider.md`.
