...
- The `src/_gt/*.json` catalogs are CLI-owned (prettierignored). `pnpm --filter <app> translate` rewrites them (needs `GT_PROJECT_ID`/`GT_API_KEY`); commit the result. Builds never depend on running it — a lagging catalog only means English text.
