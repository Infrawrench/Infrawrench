...
- **Boot order is load-bearing**: each app's entry is `src/index.ts` (not `main.tsx`), which must finish `initializeGTSPA()` before importing `./main`. Locale persistence is `localStorage["infrawrench-locale"]` via `getStoredLocale`/`setStoredLocale` from `@infrawrench/ui` — never gt-react's cookie, which the desktop's `file://` origin silently drops.
