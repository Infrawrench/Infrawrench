...
- **Releases mirror to a satellite repository**, `Infrawrench/terraform-provider-infrawrench`, exactly as the Go/Swift/PHP SDKs do — the Terraform Registry requires a dedicated repo named `terraform-provider-{NAME}` and cannot read a monorepo subdirectory. The monorepo is the direction of truth and the satellite's tree is replaced wholesale, workflows included, so **nothing there is hand-maintained**. Cutting a release is one edit: change `terraform-provider-infrawrench/VERSION` and merge. `.github/workflows/publish-terraform-provider.yml` gates on the tag `terraform-provider-v<version>`, re-verifies, mirrors, and pushes `vX.Y.Z`; the satellite's own `release.yml` (which lives in `terraform-provider-infrawrench/.github/workflows/` and is inert here) runs GoReleaser. The provider's version is **its own**, not `API_VERSION` — it is a client, so a spec bump obliges no release.
