    ...
    <li className="border border-border rounded-lg px-3 py-2">
      ...
      {field.status === "conflict" && (
        <p className="mt-1 text-xs text-warning break-all">
          Now holds <span className="font-mono">{formatChangeValue(field.current)}</span>
        </p>
      )}
      <p className="mt-1 text-xs text-on-surface-faint">{field.reason}</p>
    </li>
  );
}
...
export function RevertChangeDialog({
  ...
  return (
    <Modal onClose={onClose} ariaLabel={`Revert change to ${entry.displayName}`}>
      <div className="bg-surface-raised border border-border rounded-xl w-[min(42rem,90vw)] max-h-[80vh] overflow-auto p-5">
        <h2 className="text-lg font-semibold">Revert this change</h2>
        <p className="mt-1 text-sm text-on-surface-muted">
          {entry.displayName} — recorded {new Date(entry.createdAt).toLocaleString()}. The plan
          below is computed against the resource&apos;s current state, not against what the poller
          saw.
        </p>
        ...
        {done !== null ? (
          <div className="mt-4 text-sm text-on-surface-secondary">
            <p>
              {done.length === 1
                ? `Reverted ${done[0]}.`
                : `Reverted ${done.length} fields: ${done.join(", ")}.`}
            </p>
            <p className="mt-1 text-xs text-on-surface-faint">
              The next poll will pick this up and record it as its own change event.
            </p>
          </div>
        ) : preview === null && error === null ? (
          <p role="status" className="mt-4 text-sm text-on-surface-faint">
            Reading the resource&apos;s current state…
          </p>
        ) : plan !== null ? (
          <>
            {plan.blockedReason !== null && (
              <p className="mt-4 text-sm text-on-surface-tertiary">{plan.blockedReason}</p>
            )}
          ...
          {done === null && (
            ...
            >
              {applying
                ? "Reverting…"
                : plan
                  ? `Revert ${plan.revertibleFields.length || ""} ${
                      plan.revertibleFields.length === 1 ? "field" : "fields"
                    }`.trim()
                  : "Revert"}
            </button>
          )}
  ...
  return (
    <>
      <button
        ...
        title={refusal ?? "Put the changed fields back to what they were"}
