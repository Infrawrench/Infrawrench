...
export function BastionsSection() {
  ...
  async function handleDelete(id: string) {
    if (
      !confirm(
        gt("Revoke this bastion? Accounts that route through it will revert to direct egress."),
      )
    )
      return;
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold">{gt("Bastions")}</h1>
          <p className="text-sm text-on-surface-muted mt-1">
            {gt(
              "Outbound proxy agents you run on your own infrastructure. Bind an account to a bastion to make its cloud-API traffic exit from your IP, for source-IP allowlists or private networks.",
            )}
          </p>
        </div>
        <button
              ...
              {bastions.map((b) => (
                <tr key={b.id} className="border-b border-border/50 hover:bg-surface-raised/50">
                  ...
                  <td className="px-4 py-2 text-xs text-on-surface-muted">
                    {b.lastSeenAt ? new Date(b.lastSeenAt).toLocaleString() : gt("never")}
                  </td>
                  <td className="px-4 py-2 text-xs text-on-surface-tertiary font-mono">
                    {b.tokenPrefix}…
                  </td>
...
function CreateBastionModal({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : gt("Failed to create bastion"));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Modal onClose={onClose} ariaLabel={gt("Register a bastion")}>
        ...
        <div className="p-5 space-y-4">
          ...
          <T>
            <p className="text-xs text-on-surface-muted">
              You&apos;ll receive a one-time enrollment token and a{" "}
              <Var>
                <code>docker run</code>
              </Var>{" "}
              command to start the agent container on your infrastructure. The token cannot be
              retrieved again; copy it then.
            </p>
          </T>
          ...
          >
            {saving ? gt("Creating...") : gt("Create bastion")}
          </button>
        </div>
      </div>
    </Modal>
...
function CreatedBastionModal({
  ...
  return (
    <Modal onClose={onClose} ariaLabel={gt("Bastion created: start the agent")}>
      <div className="bg-surface-raised border border-border-strong rounded-xl w-full max-w-2xl shadow-2xl p-5">
        <h2 className="text-sm font-semibold text-on-surface-secondary mb-3">
          {gt("Bastion created: start the agent")}
        </h2>
        <T>
        ...
        <span className="block text-xs text-on-surface-muted mb-1">{gt("Run command")}</span>
        ...
        >
          {gt("Copy command to clipboard")}
        </button>
        <div className="mt-4 text-xs text-on-surface-muted space-y-1">
          <T>
            <p>
              <strong className="text-on-surface-tertiary">Next:</strong> When adding a cloud
              account, pick this bastion in the &quot;Egress via&quot; dropdown. All control-plane
              API calls for that account will route through the agent.
            </p>
          </T>
        </div>
      </div>
