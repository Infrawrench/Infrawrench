...
// --- Jira-shaped compatibility layer -----------------------------------------
//
// The Jira filing affordance shipped first, and its provider/hook were part of
// this package's public surface. They stay available as thin adapters over the
// tracker-neutral provider so an older host keeps working unchanged — a host
// that mounts `JiraFilingProvider` simply gets a filing context in which
// Linear is never configured.

/** @deprecated Use {@link IssueFilingApi}. Same shape; the name predates Linear. */
export type JiraFilingApi = IssueFilingApi;
...
/**
 * Jira-only mount of {@link IssueFilingProvider}, kept for hosts wired before
 * Linear existed. New hosts should mount `IssueFilingProvider` and pass both
...
/** The Jira-shaped view of the filing context the original hook exposed. */
export interface JiraFilingValue {
  orgId: string;
  api: IssueFilingApi;
  canFile: boolean;
  openExternal: (url: string) => void;
...
/**
 * @deprecated Use {@link useIssueFiling}, which sees both trackers. This view
 * reports Jira only — a finding filed to Linear looks unfiled through it.
 */
export function useJiraFiling(): JiraFilingValue | null {
  ...
  return useMemo(() => {
    ...
    return {
      orgId: filing.orgId,
      api: filing.api,
      canFile: filing.filableTrackers.includes("jira"),
      openExternal: filing.openExternal,
      integration: filing.jiraIntegration,
      linkFor: (sourceKind: JiraSourceKind, sourceId: string) =>
        filing.linksFor(sourceKind, sourceId).jira,
      onFiled: filing.onJiraFiled,
    };
  }, [filing]);
}
