      ...
      {pending.length > 0 && (
        <T>
          <p className="text-on-surface-faint">
            Not collected yet: <Var>{pending.map((a) => a.accountName).join(", ")}</Var>. The first
            reading lands within a few hours of adding an account.
          </p>
        </T>
      )}
      {partial.length > 0 && (
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            {QUOTA_SEVERITIES.map((severity) => (
              ...
              >
                ...
                {severity === "critical"
                  ? gt("Over {value}", { value: formatQuotaUtilization(data.threshold) })
                  : gtData(QUOTA_SEVERITY_LABELS[severity])}
              </span>
            ))}
          </div>
          ...
          {data.rows.length === 0 ? (
            ...
            <>
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                ...
                >
                  {onlyAtRisk ? gt("Showing at risk only") : gt("Show all")}
                </button>
              </div>
              ...
              {visible.length === 0 ? (
                <T>
                  <p className="text-sm text-on-surface-faint">
                    Nothing is at or heading for a limit. <Var>{data.rows.length}</Var> quota
                    <Var>{data.rows.length === 1 ? "" : "s"}</Var> tracked.
                  </p>
                </T>
              ) : (
                      ...
                      <div className="flex items-baseline justify-between gap-3">
                        ...
                        <T>
                          <span className="text-xs text-on-surface-faint">
                            <Var>{group.rows.length}</Var> quota
                            <Var>{group.rows.length === 1 ? "" : "s"}</Var>
                          </span>
                        </T>
                      </div>
                                ...
                                <td className="px-3 py-2.5 whitespace-nowrap text-right tabular-nums text-on-surface-secondary">
                                  ...
                                  <T>
                                    <span className="text-on-surface-faint">
                                      {" "}
                                      / <Var>{formatQuotaAmount(row.limit, row.unit)}</Var>
                                    </span>
                                  </T>
                                </td>
                                <td
                                  ...
                                  {row.trend.daysToExhaustion !== null ? (
                                    <T>
                                      <span
                                        ...
                                        title={gt(
                                          "Fitted over {points} readings from the last 14 days.",
                                          { points: row.trend.points },
                                        )}
                                      >
                                        full{" "}
                                        <Var>
                                          {formatDaysToExhaustion(row.trend.daysToExhaustion)}
                                        </Var>
                                      </span>
                                    </T>
                                  ) : (
