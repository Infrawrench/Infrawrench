...
<insert [The Infrastructure as Code page with the four summary tiles across the top and the resource table below, showing a mix of managed, drifted and unmanaged rows] here>
...
Attribute values are also truncated at 4 000 characters and capped at 250 attributes per resource — a value longer than that cannot be a useful diff, and the bound keeps an untrusted upload from becoming a storage problem. Uploads are limited to 8 MiB.
...
The HCL is generated by the same mappers and serializer that back [eject to Terraform](./terraform-export.md), so it is byte-identical to what "Export to Terraform…" produces for the same resources, plus the adoption blocks. Secrets are referenced as `var.*` input variables and are never inlined. Resources no plugin can express are listed with a reason rather than silently dropped.

<insert [The Adopt into Terraform modal showing generated import blocks above the resource stanzas, with the Copy HCL and Download buttons] here>
