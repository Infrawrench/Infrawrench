...
Every threshold fires at most once per calendar month. Alerts show up as a badge on the budget card, as a [mobile push notification](./mobile-push-notifications.md) to org members who have the app installed and budget alerts enabled in their per-org notification preferences, and — if your org has Twilio configured on the **Settings → Notifications** page — as an SMS to your on-call recipients.
