...
The MCP server registers ~16 generic tools that work across every plugin:
...
Every mutating tool call (`create_resource`, `delete_resource`, `invoke_action`, `apply_manifest`, the per-plugin create tools) writes a row to the [audit log](../team-and-billing/audit-log.md) with `source: "mcp"`. You can filter the audit log by source to see exactly what the model has done in your org.
