...
- **Azure** — managed disk to VM, network security group to VM primary NIC.
...
- **Azure** — load balancer backend pool to VM NIC/IP configuration, application gateway backend pool to VM or private endpoint, public IP to NIC/IP configuration, route table to subnet, NAT gateway to subnet, private DNS zone link to virtual network.
