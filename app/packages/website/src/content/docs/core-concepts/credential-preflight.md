...
<insert [Add-account modal for AWS with the credential check run: resources ✓, metrics ✓, costs ✗ with ce:GetCostAndUsage listed as missing] here>
...
<insert [Account settings page with the Check credentials button and the preflight modal showing the per-capability checklist] here>
...
<insert [Least-privilege template generator with the costs capability deselected and the generated AWS IAM policy JSON shown with a Copy button] here>
