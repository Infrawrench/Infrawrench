...
Older keys created with the deprecated `sync:read` / `sync:write` scopes are renamed automatically the next time they authenticate. The workflow permission split was handled differently, once: [workflows](../features/workflows.md) were gated on `dashboards:read` / `dashboards:write` until they got their own `workflows:read`, `workflows:write` and `workflows:approve` scopes, so keys that existed at that upgrade had the matching workflow scopes added to them there and then — they are listed on the key like any other scope, and nothing is added at authentication time. Every key created since carries exactly the scopes you chose, so grant `workflows:*` explicitly if a key needs to write, run, or approve workflows.
