...
Open a job for a **Speech** tab. It is transcription only. See [Speech testing](../features/speech-testing.md) for the panel in general.
...
<insert [Speechmatics Speech tab showing the language-pack picker populated from discovery and a completed transcript] here>

## Metrics

Jobs get a **Metrics** tab over the last 30 days charting **transcription hours** and **billable jobs**, read from `GET /v2/usage`. The window is split into at most ten buckets, each fetched as its own `since`/`until` range, because the usage endpoint takes calendar dates rather than a granularity parameter.
...
- **`au1` is batch-only.**
- **Audio, transcripts and job config are retained for 7 days.** After that the transcript endpoints return `404` and the job reports status `expired`. The Speech tab's subtitle says so, and so does the job list.
