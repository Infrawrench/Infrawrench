 ...
 *
 * Lives in client-core rather than server-core because the check is needed at
 * **both** ends. Storage is where a bad value is stopped from persisting, but
 * it is not the only way one arrives: `QuotaCapabilityDeclaration.increaseUrl`
