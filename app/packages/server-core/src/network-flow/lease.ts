 ...
 *   through a renewal this process could not confirm. The row still holds a
 *   deadline at least a full lease period past the claim, the budget runs out
 *   before that deadline can pass, so no billable query is ever issued after the
 *   point where a second replica could have taken the account.
 ...
 * renewal that does not come back — whether it never executed or committed and
 * lost its answer — leaves the collection running on a deadline at least a
 * lease period past the claim, and this budget expires before that deadline
 * can. Raise it past {@link NETWORK_FLOW_LEASE_MS} and a database outage
 * becomes a pass that keeps spending the customer's money under a lease
 ...
 *   both are this holder's until they pass. Carry on and let the next beat
 *   retry — it will match whichever of the two is there.
...
class LeaseKeeper implements NetworkFlowLease {
  ...
  #stillHeld(outcome: RenewalOutcome): boolean {
    ...
    if (outcome.status === "unconfirmed") {
      ...
      // this attempt may have committed — has not passed. Try again on the next
      // beat. If the database stays unreachable the lease simply lapses, which
      // is the safe direction — NETWORK_FLOW_MAX_RUNTIME_MS ends the pass before
      // that can happen, so nothing billable is issued under a lease this
      // process cannot confirm, and the account is freed rather than silently
      // held.
      console.error(
  ...
  #renew(): Promise<RenewalOutcome> {
    ...
    const pending = renewNetworkFlowLease(this.accountId, this.organizationId, this.#owner!)
      .then(
        (held): RenewalOutcome => (held ? { status: "renewed" } : { status: "lost" }),
