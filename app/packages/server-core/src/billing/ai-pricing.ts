 ...
 * undercharge.
 *
 * Lives in server-core because two features price with it: AI chat (web) and
 * `infra.ai()` in workflows (web + poller). `web/src/chat/pricing.ts` re-exports
 * it, so chat code keeps its import path.
 *
