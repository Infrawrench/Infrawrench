...
export function GcsBrowserPanel({
  ...
  async function handleMakeFolder() {
    ...
    try {
      await onMakeFolder(bucketName, `${prefix}${name}`);
  ...
  async function handleBulkDelete() {
    ...
    setBulkWorking(true);
    try {
      await Promise.allSettled([...selected].map((key) => onDelete!(bucketName, key)));
      reload();
    } finally {
    ...
    <div className="flex-1 min-h-0 border-t border-gray-800 bg-gray-950 flex flex-col">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-gray-800/60 flex-shrink-0">
        <div className="flex items-center gap-0.5 min-w-0 overflow-hidden flex-1">
          <span className="text-xs text-gray-500 font-medium flex-shrink-0">Files</span>
          <span className="text-xs text-gray-700 mx-1 flex-shrink-0">/</span>
          <button onClick={() => navigateTo("")} className="text-xs text-gray-500 hover:text-gray-200 flex-shrink-0 transition-colors">
            {bucketName}
          </button>
          {segments.map((seg, i) => {
            ...
            return (
              <span key={segPrefix} className="flex items-center gap-0.5 min-w-0">
                <span className="text-xs text-gray-700 flex-shrink-0">/</span>
                {isLast
                  ? <span className="text-xs text-gray-300 truncate">{seg}</span>
                  : <button onClick={() => navigateTo(segPrefix)} className="text-xs text-gray-500 hover:text-gray-200 truncate transition-colors">{seg}</button>
                }
              </span>
            );
          })}
