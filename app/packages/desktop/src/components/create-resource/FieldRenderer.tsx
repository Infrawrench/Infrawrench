...
export function FieldRenderer({ field, value, onChange }: { field: CreateFieldConfig; value: string; onChange: (v: string) => void }) {
  ...
  useEffect(() => {
    ...
    invoke<string | null>("cloud_auth_get_token")
      .then((token) => { if (!cancelled) setCloudAuthed(!!token); })
      .catch(() => { if (!cancelled) setCloudAuthed(false); });
    ...
    async function loadSystemKeys() {
      ...
      } catch { /* ssh scanning not available */ }
    }
    void loadSystemKeys();
  ...
  const generateKey = useCallback(async (name: string): Promise<SshKeyEntry & { privateKey?: string }> => {
