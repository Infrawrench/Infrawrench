...
const ALLOWED: Array<{ methods: string[]; pattern: RegExp }> = [
  ...
  // Org settings resources, one alternation per sidebar section.
  {
    ...
    pattern: new RegExp(
      `${ORG}/(team|api-keys|ssh-keys|ssh-host-keys|session-recordings|access-requests|credential-hygiene|bastions|change-freezes|tag-policy|currency|cost-centres|cost-exports|config|twilio|msteams|slack|push|digest|alert-rules|jira|linear)(\\/|$|\\?)`,
    ),
  },
