...
type AgentTool = "codex" | "claude-code";
...
ipcMain.handle(
  ...
  ) => planAgentSetup({ repoPath, tool, workspaceName }),
);
 ...
 */
async function reconcileAgentBranch({
  config,
  workspaceName,
  branchName,
  repoPath,
  ...
  const negatives = [tryLocalGit("rev-parse", "--verify", "--quiet", "HEAD"), tryLocalGit("rev-parse", "--verify", "--quiet", branchName)]
  ...
  if (result.code !== 0) {
    throw new Error(`Could not prepare the agent branch on the VM: ${stderr.trim() || stdout.trim() || `exit ${result.code}`}`);
  }
  ...
  try {
    ...
    if (currentBranch === branchName) {
      ...
      localGit("fetch", "--no-tags", bundlePath, `+refs/heads/${branchName}:refs/heads/${branchName}`);
    }
...
async function syncAgentFiles({
  ...
  for (const source of configSources) {
    ...
    if (stat.isDirectory()) {
      ...
      await uploadDirectoryArchive(config, sftpConfig, source.remotePath, source.localPath, listed.files);
