/**
 * Deployments — running a repository's Infrafile from the web app.
 *
 * All the logic lives here; the routes and the websocket session are transport
 * only, which is the same no-drift rule `services/workflows.ts` follows.
 *
...
import {
  ...
  deploymentTriggers,
  githubInstallations,
} from "@infrawrench/server-core/db/schema";
import {
  getBranchHeadSha,
