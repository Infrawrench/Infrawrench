...
export function SftpBrowser({ accountId, initialPath = "/", sshKeyId, sshHost, sshUsername }: { accountId: string; initialPath?: string; sshKeyId?: string; sshHost?: string; sshUsername?: string }) {
  ...
  }, [currentPath, refreshCount, accountId]);
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-gray-800/60 flex-shrink-0">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button onClick={() => { setNewFolderActive(true); setNewFolderName(""); setNewFolderError(null); }}
            className="px-2 py-0.5 text-xs text-gray-400 hover:text-gray-200 border border-gray-700 hover:border-gray-500 rounded transition-colors">+ Folder</button>
          <button onClick={() => fileInputRef.current?.click()}
            className="px-2 py-0.5 text-xs text-gray-400 hover:text-gray-200 border border-gray-700 hover:border-gray-500 rounded transition-colors">Upload</button>
        ...
        <div className="flex items-center gap-3 px-4 py-1.5 bg-blue-950/40 border-b border-blue-900/40 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 ml-auto">
            <button onClick={() => void handleDownload([...selected])} disabled={bulkWorking}
              className="px-2.5 py-1 text-xs text-gray-300 hover:text-white border border-gray-600 hover:border-gray-400 rounded transition-colors disabled:opacity-40">Download</button>
            {confirmBulkDelete ? (
              ...
              <button onClick={() => setConfirmBulkDelete(true)} disabled={bulkWorking}
                className="px-2.5 py-1 text-xs text-red-400 hover:text-red-300 border border-red-900/50 hover:border-red-700/50 rounded transition-colors disabled:opacity-40">Delete</button>
            )}
        ...
        {!loading && !error && (
          <table className="w-full text-xs">
            ...
            <tbody>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-3 text-gray-600">{search ? "No matches" : "Empty directory"}</td></tr>
              )}
            </tbody>
          </table>
        )}
