...
export function StorageBrowser({ accountId, bucketName }: { accountId: string; bucketName: string }) {
  ...
  const [pathInput, setPathInput] = useState(bucketName);
  ...
  useEffect(() => {
    ...
    setError(null);
    setSelected(new Set());
    storageList({ accountId, bucket: bucketName, prefix })
      .then((items) => { if (!cancelled) setObjects(items as StorageObject[]); })
      .catch((e) => { if (!cancelled) setError(formatErrorMessage(e)); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [prefix, refreshCount, accountId, bucketName]);

  useEffect(() => {
    if (!pathEditing) {
      setPathInput(prefix ? `${bucketName}/${prefix.replace(/\/$/, "")}` : bucketName);
    }
  }, [prefix, bucketName, pathEditing]);
  ...
  function toggleAll() {
    ...
    else setSelected(new Set(allFiltered.map((o) => o.key)));
  }

  const listAllFlat = useCallback(async (p: string): Promise<StorageObject[]> => {
    const items = await storageList({ accountId, bucket: bucketName, prefix: p }) as StorageObject[];
  ...
  async function handleFiles(fileList: FileList, isFolder = false) {
    ...
    await Promise.allSettled(arr.map(async (file) => {
      ...
      try {
        ...
        const resp = await fetch("/api/v1/storage/upload", { method: "POST", body: formData });
  ...
  async function handleMakeFolder() {
    ...
    try {
      await storageMakeFolder({ accountId, bucket: bucketName, key: `${prefix}${name}` });
  ...
  async function handleDelete(key: string) {
    setDeleting(true);
    try {
      await storageDelete({ accountId, bucket: bucketName, key });
  ...
  async function handleBulkDelete() {
    setBulkWorking(true);
    try {
      await Promise.allSettled([...selected].map((key) => storageDelete({ accountId, bucket: bucketName, key })));
      reload();
    } finally {
  ...
  async function handleDownload(keys: string[]) {
    ...
    try {
      ...
      const params = new URLSearchParams({
        accountId,
        bucket: bucketName,
        keys: JSON.stringify(flatKeys),
      });
      window.open(`/api/v1/storage/download?${params.toString()}`);
    } finally {
      setBulkWorking(false);
    }
  }
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 px-4 py-1.5 bg-blue-950/40 border-b border-blue-900/40 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 ml-auto">
            <button onClick={() => void handleDownload([...selected])} disabled={bulkWorking}
            ...
            {confirmBulkDelete ? (
              <span className="flex items-center gap-2">
                ...
                <button onClick={() => setConfirmBulkDelete(false)}
                  className="px-2 py-1 text-xs text-gray-500 hover:text-gray-300 transition-colors">Cancel</button>
              </span>
            ) : (
              <button onClick={() => setConfirmBulkDelete(true)} disabled={bulkWorking}
            ...
            <button onClick={() => setSelected(new Set())}
              className="text-xs text-gray-600 hover:text-gray-400 transition-colors">Clear</button>
          </div>
        </div>
      )}
              ...
              <tr className="text-gray-600 border-b border-gray-800/50">
                <th className="px-3 py-1.5 w-8">
                  <input type="checkbox" checked={allSelected}
                    ...
                    onChange={toggleAll} className="w-3 h-3 accent-blue-500 cursor-pointer" />
                </th>
            ...
            <tbody>
                  ...
                  <td className="px-2 py-1.5" colSpan={3}>
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => void handleMakeFolder()}
                        className="px-2 py-0.5 text-xs bg-blue-600 hover:bg-blue-500 text-white rounded transition-colors">Create</button>
              ...
              {prefix && !search && (
                <tr className="hover:bg-gray-800/50 cursor-pointer transition-colors"
                  onClick={() => { const parts = prefix.replace(/\/$/, "").split("/"); parts.pop(); navigateTo(parts.length ? parts.join("/") : ""); }}>
                  <td />
                  <td className="px-2 py-1.5 text-gray-500 flex items-center gap-2" colSpan={3}>
                    <span className="text-gray-600">&#8593;</span><span>..</span>
                  </td>
                </tr>
              )}
              {dirs.map((d, i) => {
                  ...
                  <tr key={d.key} className={`group transition-colors border-b border-gray-800/20 ${isSel ? "bg-blue-950/30" : "hover:bg-gray-800/50"}`}>
                    <td className="px-3 py-1.5">
                      <input type="checkbox" checked={isSel}
                        ...
                        onClick={(e) => e.stopPropagation()} className="w-3 h-3 accent-blue-500 cursor-pointer" />
                    </td>
                    ...
                    <td className="px-4 py-1.5 text-right text-gray-600">&mdash;</td>
                    <td className="px-4 py-1.5 text-right">
                      {confirmDeleteKey === d.key ? (
                        <span className="flex items-center justify-end gap-1.5">
                          <span className="text-gray-500 text-xs">Delete folder?</span>
                          <button onClick={() => void handleDelete(d.key)} disabled={deleting}
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteKey(d.key); }}
                          className="opacity-0 group-hover:opacity-100 text-gray-600 hover:text-red-400 transition-all px-1" title="Delete folder">&#10005;</button>
                      )}
                    </td>
                  </tr>
                );
                  ...
                  <tr key={f.key} className={`group transition-colors border-b border-gray-800/20 ${isSel ? "bg-blue-950/30" : "hover:bg-gray-800/30"}`}>
                    ...
                    <td className="px-4 py-1.5 text-right">
                      {confirmDeleteKey === f.key ? (
                        <span className="flex items-center justify-end gap-1.5">
                          <button onClick={() => void handleDelete(f.key)} disabled={deleting}
