...
/**
 * Fan-out SSH surface. The whole view lives in `@infrawrench/ui`
 * (`SshFanoutView`) so desktop renders the identical screen; this component
 * wires it to the org HTTP API and maps the change-freeze 423 into the shared
 * component's freeze outcome. Rendered as a workspace tab (the "ssh-fanout"
 * kind) by WebWorkspaceTabsViewport.
 */
export function WebSshFanoutPanel({ orgId }: { orgId: string }) {
