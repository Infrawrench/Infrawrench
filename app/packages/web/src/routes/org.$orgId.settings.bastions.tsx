...
export const Route = createFileRoute("/org/$orgId/settings/bastions")({
  component: BastionsPage,
});

function BastionsPage() {
  const { orgId } = useParams({ from: "/org/$orgId/settings/bastions" });
  ...
  async function load() {
    setLoading(true);
    const result = await apiGet<Bastion[]>(`/api/org/${orgId}/bastions`);
    setBastions(result);
    setLoading(false);
  }
  ...
  async function handleDelete(id: string) {
    if (
      ...
      return;
    await apiDelete(`/api/org/${orgId}/bastions/${id}`);
    await load();
  }
...
function CreateBastionModal({
  ...
  async function handleCreate() {
    ...
    setError(null);
    try {
      const result = await apiPost<CreatedBastion>(`/api/org/${orgId}/bastions`, {
        name: name.trim(),
      });
      onCreated(result);
    } catch (e) {
...
function CreatedBastionModal({
  ...
  // somewhere else. Default to the current origin for "they're running this
  // locally" demos; in production the user replaces it with the public host.
  const wsUrl = (() => {
