...
import { ApprovalsInbox } from "@infrawrench/ui";
import { createWebApprovalsClient } from "@/lib/approvals-client";
import { usePermissions } from "@/auth/permissions-context";

export const Route = createFileRoute("/org/$orgId/settings/approvals")({
  component: ApprovalsPage,
});

function ApprovalsPage() {
  const { orgId } = useParams({ from: "/org/$orgId/settings/approvals" });
  const navigate = useNavigate();
  const { has, loading } = usePermissions();
  const canRead = has("workflows:read");
  const canDecide = has("workflows:approve");

  // Stable per org so the inbox's poll effect doesn't refire every render.
  const client = useMemo(() => createWebApprovalsClient(orgId), [orgId]);
      ...
      {loading ? (
        ...
        <div className="border border-border rounded-xl overflow-hidden">
          ...
          <ApprovalsInbox
            client={client}
            canDecide={canDecide}
            onOpenWorkflow={() => void navigate({ to: "/org/$orgId/workflows", params: { orgId } })}
          />
        </div>
      )}
    </div>
