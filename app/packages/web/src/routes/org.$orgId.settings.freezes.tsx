...
function ChangeFreezesPage() {
  const { orgId } = useParams({ from: "/org/$orgId/settings/freezes" });
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      setFreezes(await apiGet<ChangeFreeze[]>(`/api/org/${orgId}/change-freezes`));
    } catch (e) {
  ...
  async function endFreeze(id: string) {
    ...
    setError(null);
    try {
      await apiPost(`/api/org/${orgId}/change-freezes/${id}/end`);
      dispatchChangeFreezeChanged();
      await load();
    } catch (e) {
  ...
  async function deleteFreeze(id: string) {
    ...
    setError(null);
    try {
      await apiDelete(`/api/org/${orgId}/change-freezes/${id}`);
      dispatchChangeFreezeChanged();
      await load();
    } catch (e) {
...
function CreateFreezeForm({ orgId, onCreated }: { orgId: string; onCreated: () => void }) {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      await apiPost(`/api/org/${orgId}/change-freezes`, {
        name: name.trim(),
