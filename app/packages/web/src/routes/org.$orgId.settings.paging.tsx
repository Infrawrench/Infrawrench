...
function MsTeamsSection({ orgId }: { orgId: string }) {
  ...
  async function load() {
    try {
      const r = await apiGet<{ webhooks: MsTeamsWebhook[] }>(`/api/org/${orgId}/msteams/status`);
  ...
  async function handleToggle(hook: MsTeamsWebhook, patch: Partial<MsTeamsWebhook>) {
    ...
    try {
      await apiPatch(`/api/org/${orgId}/msteams/webhooks/${hook.id}`, patch);
    } catch (e) {
  ...
  async function handleRemove(id: string) {
    await apiDelete(`/api/org/${orgId}/msteams/webhooks/${id}`);
    void load();
  }
  ...
  async function handleTest() {
    ...
    setTestMessage(null);
    try {
      const r = await apiPost<{ webhookCount: number; succeeded: number }>(
...
function AddMsTeamsWebhook({ orgId, onAdded }: { orgId: string; onAdded: () => void }) {
  ...
  async function handleAdd() {
    ...
    setError(null);
    try {
      await apiPost(`/api/org/${orgId}/msteams/webhooks`, { label, url });
      setLabel("");
      setUrl("");
      onAdded();
    } catch (e) {
 ...
 * by the "Weekly digest" checkbox on each Slack channel and Teams webhook
 * above — this section only turns the schedule on and proves the pipeline
 * with a "Send now".
 */
function WeeklyDigestSection({ orgId }: { orgId: string }) {
