...
describe("gcp node-driver", () => {
  ...
  describe("downloadFile", () => {
    it("downloads file successfully on HTTP 200", async () => {
      ...
      mockHttpsGet.mockImplementation((_url: string, _opts: unknown, cb: Function) => {
    ...
    it("constructs correct GCS URL with encoded bucket and key", async () => {
      ...
      mockHttpsGet.mockImplementation((url: string, _opts: unknown, cb: Function) => {
        const req = new EventEmitter();
        const res = createMockResponse(200);
        res.pipe = vi.fn(() => {
          process.nextTick(() => mockWs.emit("finish"));
    ...
    it("passes Authorization header with bearer token", async () => {
      ...
      mockHttpsGet.mockImplementation((_url: string, opts: any, cb: Function) => {
        const req = new EventEmitter();
        const res = createMockResponse(200);
        res.pipe = vi.fn(() => {
          process.nextTick(() => mockWs.emit("finish"));
