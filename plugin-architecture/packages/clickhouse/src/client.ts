...
export class ClickHouseClient implements PluginClient {
  ...
  // ---------------------------------------------------------------------------
  // Cloud API helper (manages services, orgs, etc.)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // ClickHouse HTTP Interface helper (runs SQL queries)
  // ---------------------------------------------------------------------------

  private async chQuery(sql: string): Promise<Record<string, unknown>[]> {
    if (!this.chHost) return [];

    const host = this.chHost.startsWith("https://") ? this.chHost : `https://${this.chHost}`;
  ...
  private renderServiceDetail(
    ...
    const dotStatus = ClickHouseClient.STATE_MAP[state] ?? "unknown";
  ...
  renderSidebarItem(resource: ResourceInstance): SidebarItemSchema {
    ...
    return {
      ...
      status: {
        kind: "status-dot",
        status: ClickHouseClient.STATE_MAP[state] ?? "unknown",
      },
    };
  }

  // ---------------------------------------------------------------------------
  // PluginClient — SQL execution via ClickHouse HTTP Interface
  // ---------------------------------------------------------------------------
