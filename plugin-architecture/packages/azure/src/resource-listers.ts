...
//
// Azure Resource Manager API version is specified per-resource-type.
...
export async function listServiceBusNamespaces(
  ...
  const data = await ctx.get<{ value: Record<string, unknown>[] }>(
    `${ARM}/subscriptions/${ctx.subscriptionId}/providers/Microsoft.ServiceBus/namespaces?api-version=2022-10-01-preview`,
  );
...
export async function listDNSZones(
  ...
  const data = await ctx.get<{ value: Record<string, unknown>[] }>(
    `${ARM}/subscriptions/${ctx.subscriptionId}/providers/Microsoft.Network/dnszones?api-version=2023-07-01-preview`,
  );
...
export async function listEventHubNamespaces(
  ...
  const data = await ctx.get<{ value: Record<string, unknown>[] }>(
    `${ARM}/subscriptions/${ctx.subscriptionId}/providers/Microsoft.EventHub/namespaces?api-version=2022-10-01-preview`,
  );
  return (data.value ?? []).map((ns) => {
    const name = String(ns["name"] ?? "");
    const azureId = String(ns["id"] ?? "");
