/**
 * PlanetScale public status feed (incident.io emulating the Statuspage v2
 * API, https://www.planetscalestatus.com — verified 2026-08).
 *
 * incident.io hosts 404 on /incidents/unresolved.json, so this fetches
 * /api/v2/incidents.json (full history) and filters to unresolved after
 * parsing. Incident objects carry no components — the parser marks them
 * provider-wide automatically. The component mapper is future-proofing:
 * components on this page are cloud regions like "us-east-1 (Northern
 * Virginia)" or "us-central1 (Iowa)" (occasionally with double spaces
 * before the parenthesis) plus service components ("Database Operations",
 * "Dashboard and API Requests", "Query Insights").
 */
import type {
  StatusComponentMapping,
  StatusFeedDeclaration,
  StatusIncident,
} from "@infrawrench/plugin-base";
import { parseStatuspageIncidents } from "@infrawrench/plugin-base";

export const statusFeed: StatusFeedDeclaration = {
  url: "https://www.planetscalestatus.com/api/v2/incidents.json",
  format: "statuspage-v2",
  statusPageUrl: "https://www.planetscalestatus.com",
};

/** "us-east-1 (Northern Virginia)" / "us-central1  (Iowa)" → leading slug. */
const REGION_COMPONENT = /^([a-z0-9-]+)\s*\(/;

function mapComponent(name: string): StatusComponentMapping | null {
  const region = name.match(REGION_COMPONENT);
  if (region?.[1]) return { regions: [region[1]] };
  return { services: [name] };
}

export function parseStatusFeed(body: string): StatusIncident[] {
  // The feed is full history — keep only active incidents; the host closes
  // cached rows that disappear from the parsed set.
  return parseStatuspageIncidents(body, {
    mapComponent,
    statusPageUrl: statusFeed.statusPageUrl ?? statusFeed.url,
  }).filter((incident) => incident.state !== "resolved" && !incident.resolvedAt);
}
