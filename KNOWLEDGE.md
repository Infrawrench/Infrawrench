...
**Mobile renders the same widgets, not a pointer to the web app.** The home tab is the org's default dashboard (`GET /dashboards/default/full`, the same call the web home route makes) and `mobile/src/features/dashboard/DashboardBody.tsx` renders resource pins, workflow pins, cost graphs, and budgets in `orderDashboardCards` order — there is no standing "Budgets" section on the home screen any more, because a budget belongs to the dashboard it was created on. The home screen does append the budgets the default dashboard doesn't already show — a `budget_breach` push routes to `/org/:orgId` (`pushDataToPath`), so a budget whose widget sits on another dashboard, or whose widget was removed while the row lived on, must not be unreachable from the screen the notification opens. Its earlier standalone version also read `status.actualCents`, a shape `GET /budgets` never returned (actual/forecast/month are top-level), so every budget rendered at $0: that class of drift is why the contract now lives in client-core rather than being re-declared per screen.
