...
Hosted Claude-powered chat — same tool registry as MCP (resource lifecycle, connection-layer tools, cost/budget tools). The agent loop lives entirely server-side. **Web-only surface**: the desktop app has no chat route (chat needs the cloud Postgres tables, billing, and server-side agent loop; the desktop's local SQLite has none of that). The per-resource `ChatPanel` in `@infrawrench/ui` is a separate feature and exists on both.

- **Code:** `app/packages/web/src/chat/agent.ts` (agent loop + suspend/resume), `chat/auth.ts` (session cookie OR WorkOS Bearer OR `iwk_` API key with `chat:write`), `chat/billing.ts` (per-turn `chat_usage` rows + Stripe metered usage), `chat/pricing.ts` (per-Mtok rates × markup, env-overridable). Routes: `api/routes/chat.ts`. UI: `routes/org.$orgId.chat.tsx` (layout with `Outlet` — required for the `$conversationId` child to render) + `routes/org.$orgId.chat.index.tsx` (list) + `routes/org.$orgId.chat.$conversationId.tsx`, components in `components/chat/`. The sidebar (`components/WebSidebar.tsx`) lists recent sessions under a "Chat" section (refreshes via the `CHAT_CONVERSATIONS_CHANGED_EVENT` window event from `lib/chat-events.ts`).
