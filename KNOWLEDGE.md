...
Hosted Claude-powered chat with full UI parity — same tool registry as MCP (resource lifecycle, connection-layer tools, cost/budget tools). The agent loop lives entirely server-side; web + desktop both render the same React route.
