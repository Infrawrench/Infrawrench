...
**Cost anomaly alerts** — statistical spend-spike detection beside budgets, zero-config. Pure maths in `server-core/src/cost/anomaly-detect.ts` (`detectSpike`: mean + 3σ over the trailing 28 days, ≥$10 absolute rise, ≥7 observed days — unit-tested, no I/O); the evaluator `cost/anomaly-eval.ts` `detectCostAnomaliesForOrg` runs from `poller/src/cost-poll.ts` right after `evaluateBudgetsForOrg`, evaluates **yesterday** (today is always partial and reads as a dip) per provider and per service via two grouped `queryCosts` calls, per currency. Dedup is two-layer: `cost_anomalies` unique (org, day, dimension, dimensionKey, currency) + `onConflictDoNothing`/RETURNING (the budget-events pattern; migration `0047`), and a 7-day per-key cooldown that stores but doesn't re-notify sustained level shifts. Fan-out through the existing transports under a NEW `PushTrigger` `"anomalyAlerts"` — which means an `anomaly_alerts` boolean on `push_preferences`, `slack_channels`, AND `msteams_webhooks`, plus the trigger-column maps in `push/dispatch.ts`, `slack.ts`, `msteams.ts` (adding a trigger touches all six). No Twilio paging for anomalies (deliberate). Push payload `type: "cost_anomaly"` routes mobile to `/org/:orgId/costs`. Read side: `GET /api/org/:orgId/costs/anomalies?days=` (`costs:read`, web `services/cost-anomalies.ts`, API*VERSION → 0.19.0); `CostAnomaly` contract in `client-core/src/costs.ts`; `ui/src/cost/CostAnomaliesSection.tsx` renders on `CostsPanel` for web+desktop, gated on the host wiring the optional `CostsClient.listAnomalies` (desktop: `cloud_costs_anomalies` IPC). Toggles live wherever budget toggles live: web paging settings (`ALERT_TRIGGERS`), mobile notifications screen. Mobile anomalies \_list* is a follow-up.
