...
- **Surfaces**: shared `MomentPanel` in `ui/src/moment/` (host-injected `MomentClient`, Changes-template): datetime-local input + "Around now" + ±15m/±1h/±6h presets, feed-health chips ("Workflow runs unavailable"), incident-overlap box, timeline with burst expansion; `onQueryChange` lets hosts mirror `at`+`window` into the URL (deep-linkable/shareable — both web `org.$orgId.moment.tsx` and desktop `routes/moment.tsx` use `validateSearch` + `replace: true` navigation). Entered via an optional `onInvestigateMoment` button on `ChangesPanel`. Desktop rides a new `cloud_moment` IPC (`electron/cloud-data/moment.ts`, preload allowlist); plain route, not a workspace tab, same reasoning as Changes; audit/freeze links deliberately no-op there (no native screens). Mobile is `features/moment/` (native rewrite, `useMoment` react-query hook, route `org/[orgId]/moment` + hidden tab) — and `pushDataToPath` now lands `cost_anomaly`, `provider_incident` and `resource_drift` pushes on the moment view (drift computes centre = midpoint of `since→now` and window to cover it; the Changes screen still honours old `since` links). CLI: `infrawrench moment [timestamp]` (`cli/commands/moment.ts`), `-w/--window 30m|6h` via `parseDuration`, `--json` or a glyph-per-severity text timeline. MCP: `what_changed` (`web/src/tools/moment.ts`) — declared permission `resources:read` floor, then per-feed gating via `effectiveToolPermissions`. Docs: `features/moment.md`.
...
- **Surfaces**: shared `MetricAlertsPanel`/`MetricAlertRuleModal` in `ui/src/metric-alerts/`; web plain route `org.$orgId.metric-alerts.tsx` + sidebar "Alerts" tile; desktop plain route `/metric-alerts` (cloud-only tile, `cloud_metric_alerts_*` IPC channels); mobile gets the push toggle + Slack/Teams trigger rows only (no rule editor — the read-only-billing line); CLI `infrawrench alerts` / `alerts events` (cloud-only, `--json`, pure formatters in `cli/format.ts` because client-core imports must stay type-only). Docs: `features/metric-alerts.md`.
