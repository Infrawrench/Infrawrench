...
- **Follow-ups** — mobile surface (deliberate omission for now); the CLI has no `--local` half, since desktop's local graph assembly lives in the renderer (`src/lib/local-dependency-graph.ts`) over the renderer DB client.
...
same failure class. Tested end-to-end in `__tests__/digest-retry.test.ts` against an in-memory DB
