...
- **Pricing:** per-model Anthropic passthrough rates in `chat/pricing.ts` (Opus 4.8 $5/$25, Sonnet 5 $3/$15, Haiku 4.5 $1/$5 per Mtok; cache write 1.25×, cache read 0.1× of input) × `INFRAWRENCH_CHAT_MARKUP` (default 1.5). Unknown models fall back to Opus rates. The old single-rate `INFRAWRENCH_CHAT_PRICE_*_PER_MTOK` env overrides are gone. Each turn's tokens are persisted as `chat_usage(cost_micros)`; if `INFRAWRENCH_STRIPE_CHAT_METER_EVENT` is set we also push a Stripe meter event keyed by customer.
