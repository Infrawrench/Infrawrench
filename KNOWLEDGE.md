...
- One-time interactive setup on a workstation before CI can pass: `eas device:create` for at least one device, then `eas credentials` to let EAS generate and store the iOS distribution cert + ad-hoc profile.
