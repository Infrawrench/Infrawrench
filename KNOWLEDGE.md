...
- **`rdpEndpoint`** — Remote Desktop for Windows VMs; parallel to `sshEndpoint` (`hostOutputKey`, `privateHostOutputKey?`, `runningWhen?`, `defaultUsername?`, `usernameFieldKey?`) plus `windowsWhen?: { fieldKey, value }` to gate on the OS so Linux VMs don't grow an RDP button. Declared by aws `ec2-instance` (gate `platform === "windows"`, default user `Administrator`), azure `azure-vm` (gate `osType === "Windows"`), gcp `gce-instance` (gate `osFamily === "windows"`, derived from the boot disk's license URIs). The OS signal is provider metadata the listers already fetch — never the instance name. Host resolution mirrors SSH: desktop local `-loaders/local.ts`, web `resource-detail.ts` (emits `hasRdp`/`rdpHost`/`rdpPrivateHost`/`defaultRdpUsername`). **RDP works in desktop AND web** — both render `ironrdp-wasm` (WASM build of IronRDP) on a canvas; they differ only in transport and file I/O:

- **Shared code**: `@infrawrench/rdp-core` (the RDCleanPath DER codec + TCP/X.224/TLS handshake + relay over an abstract `RdpChannel`, with a `destinationOverride`); `@infrawrench/client-core` `scancodeFor` + `RdpFileTransferManager` (the CLIPRDR protocol, parametrized by `RdpUploadFile`/`RdpFileSink` adapters).
- **Desktop**: the WASM's `WebSocket` is shimmed (`src/lib/rdp-transport.ts`) to tunnel frames over IPC to `electron/rdp-host.ts`, which runs `rdp-core` — **no listening socket**. File I/O = `rdp_local_file_*` / `rdp_save_download` IPC (dialog-blessed paths). CLI `infrawrench rdp <id>` resolves + validates headlessly and hands off to the GUI via an `infrawrench://rdp?account=…&resource=…` deep link (buffered in `cloud-auth.ts`, consumed by `__root.tsx`).
- **Web**: the browser opens a real `wss://…/api/rdp?token&account&resource` to a dedicated server socket (`server.ts` upgrade branch → `services/rdp-proxy.ts`). The server resolves the destination from the authenticated resource (`services/rdp.ts`, applies the running+Windows gates) and passes it as `destinationOverride` — the browser's RDCleanPath destination is **ignored** (SSRF), and `assertHostNotInternal` guards it like the SSH proxy. File I/O = browser `File.slice()` upload / anchor-download.
- Deliberately **not** on mobile (would need a native client). RDP passwords are never stored, prompted every session.
