...
- **Models:** per-conversation, picked at create time. Supported list is `CHAT_MODELS` in `@infrawrench/ui` (`claude-opus-4-8` default, `claude-sonnet-5`, `claude-haiku-4-5`); the create route rejects anything else. Legacy conversations on `claude-sonnet-4-6` keep working (still priced).
