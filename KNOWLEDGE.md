...
`GlobalTabBar` takes a `trailingActions` slot for controls that are chrome rather than tabs. It is rendered outside the scrolling tab strip — the strip, not the bar, is what carries `overflow-x-auto`, so an action stays reachable with any number of tabs open — and supplying it also keeps the bar mounted when there are no tabs, since a permanent control can't live in a bar that returns `null`. That last part is why `role="tablist"` sits on a wrapper around the tabs alone rather than on the strip: with the bar now staying mounted, web reaches the no-pinned-tabs state on every org route, and a tablist owning zero tabs (or owning the non-tab "+" button) is invalid ARIA. Web puts **Settings** there. It used to be a button at the foot of the sidebar, below the account tree, where it read as part of that list and was routinely missed; settings is not a workspace tab (it renders through the layout `<Outlet/>`, with `syncWorkspaceRouteFromPath` returning `null` so the viewport hides every panel), so a control beside the tabs is the honest shape for it. Desktop passes nothing — org settings are a cloud-only surface and the desktop app has no settings route.
...
## Dependency graph (output references as topology)

The org-wide "Graph" page and the per-resource "Dependencies" tab are both drawn from the output-reference link data — no plugin contributes edges, so the feature is fully generic over providers.

- **Model/layout (pure, shared)** — `client-core/src/dependency-graph.ts`: `buildDependencyGraph` (dedupes edges by `(consumer, fieldKey)` — the `associations` row and the `secret_field_states` output-ref row describe the same reference — and drops dangling/self edges), `directDependencies` (the Depends on / depended on by lists), `collectDependents` (blast radius, includes the start node), `collectDependencies`, and `layoutDependencyGraph` (longest-path layering, providers left, plus two barycenter sweeps; cycle-safe). Re-exported from `@infrawrench/ui` per the shared-contract rule.
- **Server** — `GET /api/org/:orgId/dependency-graph` (`web/src/api/routes/dependency-graph.ts`, `resources:read`): unions org-scoped `associations` with `resolution_kind = 'output-ref'` `secret_field_states` rows, returns only nodes participating in ≥1 edge (the graph is wiring, not inventory) with plugin logo/type labels resolved like `search.ts`. `?resourceId=` narrows to one resource's direct neighbourhood — the Dependencies tab asks for that and discards nothing, where the org-wide answer would have it pull the whole topology on every detail-page mount; the filter is pushed into SQL, and the focused path fetches resource rows by id after the edges land rather than scanning the org. OpenAPI `paths/dependency-graph.ts`; shipped as API **0.25.0**.
- **UI** — `ui/src/components/graph/`: `DependencyGraphView` (hand-rolled SVG, no graph library; arrows point consumer → provider; click = select + highlight transitive dependents, click again = open resource; theme-aware via `var(--color-*)` since Tailwind utilities don't reach SVG attrs), `ResourceDependenciesPanel`, `GraphIcon`. `DetailView` grew optional `dependencies`/`onOpenDependency` props → a "Dependencies" tab that only appears when the resource has ≥1 edge.
- **Hosts** — new workspace tab kind `graph` (store id/title/equality + `graphTabTarget` + both hosts' navigate-args/path-sync/viewport/sidebar tiles, plus the two `validate-tabs` kind lists — server route and openapi enum). Web: `WebGraphPanel` fetches the endpoint; `ResourceDetailClient` fetches it per-resource and filters with `directDependencies`. Desktop: the tile shows in BOTH modes (unlike Costs) — cloud mode proxies over new `cloud_dependency_graph` IPC, local mode assembles the same shape from local SQLite in `src/lib/local-dependency-graph.ts` (same tables, same dedupe rule); `-DetailViewContainer.tsx` wires the per-resource tab the same dual-mode way. Adding a tab kind means hand-editing both `routeTree.gen.ts` files if you don't run the router codegen.
