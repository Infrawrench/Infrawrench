...
- **Server** — `GET /api/org/:orgId/dependency-graph` (`web/src/api/routes/dependency-graph.ts`, `resources:read`): unions org-scoped `associations` with `resolution_kind = 'output-ref'` `secret_field_states` rows, returns only nodes participating in ≥1 edge (the graph is wiring, not inventory) with plugin logo/type labels resolved like `search.ts`. OpenAPI `paths/dependency-graph.ts`; shipped as API **0.25.0**.
