...
- **`listFiles` is capped at 2,000** (`MAX_LISTED_FILES`, 4 pages of 500). The endpoint will page through six figures, and one resource row per file per poll cycle is not a trade worth making. The app's own `filesUploaded` is the honest total, and the app detail view says out loud when it exceeds what was synced — a short list that silently stands in for 40,000 files is worse than no list. The storage browser shares the cap.
...
- `ut-app` sets `hiddenChildTypeIds: ["ut-file"]` rather than a `childTables` entry. The storage-browser tab already _is_ the file listing, with upload/download/delete on the same rows; an Overview copy is a second table that disagrees with it by definition once an app passes `MAX_LISTED_FILES` (the browser pages the provider, the table shows what synced).
