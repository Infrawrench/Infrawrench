...
**Desktop app** — Electron + Vite + React, local SQLite, works offline. All 48 provider plugins loaded. SSH terminals, SQL editors, K8s exec, SFTP browsers run locally.

**Web app** — Hono server (Node) + Vite/React frontend with TanStack Router, Neon PostgreSQL via Drizzle ORM, WorkOS auth. All 48 plugins loaded server-side. SSH/SQL/K8s proxied through a custom WebSocket server (`server.ts`).
...
- `SidebarDashboards.tsx` — dashboard list + create, under the org tile grid (Agents, Workflows, Deploy, Costs, Alerts, Changes, Graph, Expiring, Posture, Probes, Fan-out, Logs — each one a workspace-tab kind); dashboard sidebar pill/drop/drag visuals are canonicalized in shared `@infrawrench/ui` `DroppableDashboardItem` so desktop and web stay in sync
...
**Drift alerts** (follow-up to the above; migration `0054`). A fifth `PushTrigger` `"resourceDrift"` fans the change feed out over push/Slack/Teams — so `resource_drift` booleans on `push_preferences`, `slack_channels` and `msteams_webhooks` plus the trigger-column maps in `push/dispatch.ts`, `slack.ts`, `msteams.ts` (adding a trigger touches all six files, as the anomaly entry says). It **defaults off** on all three, alone among the triggers: anomalies could default on because 3σ + a cooldown makes them rare by construction, whereas drift is continuous — a healthy busy org produces it constantly — and defaulting on would have started posting into every already-connected channel the moment the migration landed.
...
- **EAS fails on `tsdown.config.ts` unless `unrun` is installed at the workspace root.** `tsdown`'s auto config loader is `native` only when `process.features.typescript` is set (Node ≥ 22.6 with type stripping); otherwise it is always `unrun` — even for `.mjs` configs. SDK 54 images default to Node 20.19.x, so `eas-build-post-install` dies with `Failed to import module "unrun"` while local/CI Node 24 succeeds. `unrun` is an optional peer of `tsdown` and is never auto-installed; add it as a root `devDependency` so Node's walk-up resolution from `.pnpm/tsdown@…` reaches `node_modules/unrun`. `eas.json` `base` also pins `node: "22.18.0"` + `pnpm: "9.15.4"` (tsdown engines `^22.18.0 || >=24.11.0`; packageManager is `pnpm@9.15.4`) so the worker is in-range even if type stripping is off.
