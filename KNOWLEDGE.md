...
**Desktop app** — Electron + Vite + React, local SQLite, works offline. All 47 provider plugins loaded. SSH terminals, SQL editors, K8s exec, SFTP browsers run locally.

**Web app** — Hono server (Node) + Vite/React frontend with TanStack Router, Neon PostgreSQL via Drizzle ORM, WorkOS auth. All 47 plugins loaded server-side. SSH/SQL/K8s proxied through a custom WebSocket server (`server.ts`).
