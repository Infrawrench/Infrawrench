...
**Renaming a permission family is a compat problem, not a rename.** Workflows rode on `dashboards:read`/`dashboards:write` until they got `workflows:read` / `workflows:write` / `workflows:approve` (run sits under `write` — an editor can add a cron trigger anyway; `approve` is separate so a custom role can grant authorship without sign-off). System roles resolve from code so they upgrade for free, but the two places that store grants verbatim — `roles.permissions` for custom roles and `api_keys.scopes` — would silently lose access. `server-core/src/permissions/legacy-workflows.ts` handles both: `grandfatherWorkflowPermissionsIfLegacy(granted, writtenAt)` additively expands `dashboards:*` onto the workflow family when the grant was written before `WORKFLOW_PERMISSION_CUTOVER`, called from `resolveEffectivePermissions` (custom roles, keyed on `roles.updatedAt`, which the role PATCH bumps) and from `migrateScopes` in `auth/api-auth.ts` (keyed on `api_keys.createdAt`, and persisted by the existing scope write-back). No migration involved. Delete the module once the cutover is well past. Custom graphs deliberately stayed on `dashboards:*` — they really are dashboard content.
